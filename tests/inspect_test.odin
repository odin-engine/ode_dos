/*
    ...
    Tests for inspecting designed objects and for the command-line tooling.
*/
...
// Inspect
    ...
    inspect__golden__test :: proc(t: ^testing.T) {
        ...
        testing.expect(t, dos.load(&g.cfg, "../samples/thief_game/data") == nil)
        guard, found := dos.find(&g.cfg, "bafford.Guard01")
        testing.expect(t, found)
        if !found do return

        b := strings.builder_make(context.temp_allocator)
        dos.dump(&g.cfg, guard, strings.to_writer(&b))
